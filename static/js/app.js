// Anusha Submission page Module 14 -  url with data
const url = "https://2u-data-curriculum-team.s3.amazonaws.com/dataviz-classroom/v1.1/14-Interactive-Web-Visualizations/02-Homework/samples.json"

//Use the D3 library to read data 
...
// Make the  panel for demographics
function demo(selectedValue) {
  ...
  function bar_bubble(selectedValue) {
    // Fetch the JSON data and console log it
    d3.json(url).then((data) => {
        ...
        // Filter data for selected value 
    ...
    d3.json(url).then((data) => {
       ...
       let obj = filteredData[0]
        ...
        let trace3 = [{
            ...
            gauge: {
                axis: {range: [null, 10]}, 
                bar: {color: "rgb(100,200,400)"},
                steps: [
                    { range: [0, 1], color: "rgb(230,245,248)" },
                    { range: [1, 2], color: "rgb(220,237,244)" },
                    { range: [2, 3], color: "rgb(218,230,239)" },
                    { range: [3, 4], color: "rgb(180,223,238)" },
                    { range: [4, 5], color: "rgb(170,216,230)" },
                    { range: [5, 6], color: "rgb(150,209,225)" },
                    { range: [6, 7], color: "rgb(143,202,221)" },
                    { range: [7, 8], color: "rgb(128,195,216)" },
                    { range: [8, 9], color: "rgb(113,187,212)" },
                    { range: [9, 10], color: "rgb(100,150,207)" }
                ]
            }
        }];
